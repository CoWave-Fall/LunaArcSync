...
class PageRepository implements IPageRepository {
  ...
  Future<page_models.Page> createPage({
    ...
    try {
      ...
      final response = await _apiClient.dio.post('/pages', data: formData);
  ...
  Future<page_models.PageDetail> getPageById(String pageId) async {
    try {
      final response = await _apiClient.dio.get('/pages/$pageId');
  ...
  Future<JobQueuedResponse> startOcrJob(String versionId) async {
    try {
      final response = await _apiClient.dio.post('/jobs/ocr/$versionId');
  ...
  Future<List<page_models.PageVersion>> getVersionHistory(String pageId) async {
    try {
      final response = await _apiClient.dio.get('/pages/$pageId/versions');
  ...
  Future<void> revertToVersion({required String pageId, required String targetVersionId}) async {
    try {
      ...
      await _apiClient.dio.post('/pages/$pageId/revert', data: requestBody);
    } on DioException catch (e) {
  ...
  Future<List<page_models.Page>> getUnassignedPages() async {
    try {
      final response = await _apiClient.dio.get('/pages/unassigned');
  ...
  Future<void> deletePage(String pageId) async {
    try {
      await _apiClient.dio.delete('/pages/$pageId');
    } on DioException catch (e) {
      if (e.response?.statusCode == 404) {
        throw Exception('Page not found.');
      }
  ...
  Future<page_models.PageVersion> addVersionToPage({
    ...
    try {
      ...
      final response = await _apiClient.dio.post('/pages/$pageId/versions', data: formData);
