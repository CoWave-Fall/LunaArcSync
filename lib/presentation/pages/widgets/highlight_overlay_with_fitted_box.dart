...
class HighlightOverlayWithFittedBox extends StatelessWidget {
  final List<Bbox> bboxes;
  final int imageWidth; // Original OCR image width
  final int imageHeight; // Original OCR image height
  final double renderedImageWidth; // Actual rendered width of the image
  final double renderedImageHeight; // Actual rendered height of the image
  final Size containerSize; // Container size for FittedBox calculation
  final Color highlightColor;

  const HighlightOverlayWithFittedBox({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: bboxes.map((bbox) {
        ...
        final rect = CoordinateConverter.convertToDisplayRectWithFittedBox(
          bbox, 
          imageSize, 
          displaySize, 
          containerSize
